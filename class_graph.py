        ...
        for i in range(len(list_of_succesive_coefficients[0, :])):
            if i < self.M:
                ...
                title = " Histogram, estimator (time = {}) of the parameter BETA, {} tries, {}th value, true value = {}.".format(
                    self.T_max,
                    self.nb_of_guesses, i + 1 - self.M - self.M * self.M, np.ravel(self.BETA)[i - self.M * self.M - self.M](0,1)    )
                range_coef = (0.5, 1.5)
                ecart_plot = tuple(inner * np.ravel(self.BETA)[i - self.M - self.M * self.M](0,1)  for inner in range_coef)
...
\nOnly 10-90% of the interval is shown (boundary effect), starting from 0 until {}.".\
    ...
    def MSE_convergence_estimators_limit_time(self, mini_T):
        ...
        estim_alpha.function_upon_separeted_data("value", useful_functions.compute_MSE, "compute_MSE", true_parameter = estim_alpha.DF["true value"].mean() )
        estim_beta.function_upon_separeted_data("value", useful_functions.compute_MSE, "compute_MSE", true_parameter = estim_beta.DF["true value"].mean())
        estim_nu.function_upon_separeted_data("value", useful_functions.compute_MSE, "compute_MSE", true_parameter = estim_nu.DF["true value"].mean())
        ...
        plot_functions.plot_graph(TIMES_plot, MSE_reals, title=[
            "Convergence in compute_MSE of the estimators, batches of {} realisations.".format(self.nb_of_guesses)],
                                  ...
                                  parameters=[self.ALPHA[0][0](0,1), self.BETA[0][0](0,1), self.NU[0](0,1)], name_parameters=["ALPHA", "BETA", "NU"],
