    ...
    while np.linalg.norm(derivative, 2) > e and step > tol or number_of_step_crash == 0:  # I use norm 2 as criterea
        ...
        if not silent:
            if number_of_step_crash % 10 == 0:
                print("Step Newton {} result {}, change in norm {}.".format(number_of_step_crash, np.linalg.norm(derivative, 2), step ))
                print( "                                             GUESSES : \n ALPHA : \n {}  \n BETA : \n {} \n NU : \n {}".format( ALPHA,BETA,MU ) )
        ...
        if number_of_step_crash - reset_index < 10 and np.linalg.norm(derivative, 2) > 50*M*M and nb_of_explosions < 2 :
            multi = 1/ M**3
        elif number_of_step_crash - reset_index < 10 and np.linalg.norm(derivative, 2) > 2*M*M and nb_of_explosions < 5 :
            multi = 0.6/ M**3
...
def armijo_rule(f, df, x0, direction, a, sigma, b):
    ...
    print("we are in ARMIJO condition because too many steps, the ok directions and step :" + str(condition) + " and " + str( a ) )
...
def estimation_hp(hp, estimator, T_max, kernel_weight = kernel_plain, time_estimation=0, silent = True):
    ...
    while not flag_success_convergence:
        ...
        flag_success_convergence, alpha_hat, beta_hat, mu_hat = functions_MLE.call_newton_raph_MLE_opt(time_real, T_max, w, silent = silent)
...
def multi_estimations_at_one_time(hp, estimator, T_max, nb_of_guesses, kernel_weight = kernel_plain, time_estimation=0, silent = False):
